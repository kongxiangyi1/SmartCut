    ...
    try:
        ...
        try:
            ...
            project_data = ProjectCreate(
                ...
                status=ProjectStatus.PENDING,  # 初始状态为等待"
...
async def process_youtube_download_task(task_id: str, request: YouTubeDownloadRequest, project_id: str):
    ...
    try:
        ...
        ydl_opts = {
            ...
            'writeautomaticsub': True,  # 下载自动生成的字'
            'subtitleslangs': ['en', 'zh-Hans', 'zh', 'en-US', 'auto'],  # 英文和中文字幕，包括自动检'
            'subtitlesformat': 'srt',
        ...
        if not subtitle_path:
            ...
            try:
                ...
                await update_project_download_progress(project_id, 90.0, "字幕生成完成，正在准备处")
                ...
                try:
                    ...
                    if subtitle_path:
                        ...
                        logger.warning("所有字幕获取策略都失败")

                        subtitle_path = None  # 确保字幕路径为空，后续会标记项目失败

                except Exception as backup_error:

                    logger.error(f"备用字幕获取也失: {backup_error}")

                    subtitle_path = None  # 确保字幕路径为空，后续会标记项目失败

            except Exception as e:

                logger.error(f"生成字幕过程中发生未知错: {e}")
        ...
        try:
            ...
            if not project:
                ...
                raise Exception(f"项目 {project_id} 不存")
            ...
            # 检查字幕文件是否存在，如果不存在则标记项目为失"
            srt_file_path = raw_dir / "input.srt"

            if not srt_file_path.exists():

                logger.error(f"字幕文件不存在 {srt_file_path}，项目将标记为失败状")
                ...
                logger.info(f"YouTube下载任务失败: {task_id}, 项目ID: {project.id}, 原因: 字幕文件不存")
            ...
            # 更新项目下载进度为完"
            await update_project_download_progress(project_id, 100.0, "下载完成，准备开始处")
            ...
            try:
                ...
                project.status = ProjectStatus.PENDING  # 改为PENDING，让自动化服务启"
                db.commit()

                

                logger.info(f"YouTube项目 {project.id} 下载完成，等待自动化流水线启")
                ...
                if pipeline_result['status'] == 'started':
                    ...
                    logger.info(f"YouTube项目 {project.id} 自动化流水线已启: {pipeline_result}")
...
async def _try_extract_from_metadata(url: str, download_dir: Path, browser: Optional[str] = None) -> str:
    ...
    logger.info("尝试从YouTube视频元数据提取字幕信")
    ...
    try:
        ...
        logger.debug(f"提取YouTube视频元数据失: {e}")
