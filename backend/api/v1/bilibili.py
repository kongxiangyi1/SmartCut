...
async def parse_bilibili_video(
    ...
    try:
        ...
        if not downloader.validate_bilibili_url(url):
            ...
            raise HTTPException(status_code=400, detail="无效的B站视频链")
...
async def create_bilibili_download_task(request: BilibiliDownloadRequest):
    ...
    try:
        ...
        logger.info(f"创建B站下载任: {request.url}")
        ...
        try:
            ...
            project_data = ProjectCreate(
                ...
                description=f"从B站下: {video_info.title}",
...
async def process_download_task(task_id: str, request: BilibiliDownloadRequest, project_id: str):
    ...
    try:
        ...
        await update_project_download_progress(project_id, 60.0, "视频下载完成，正在处理字")
        ...
        if not subtitle_path and video_path:
            ...
            try:
                ...
                # Whisper失败时，标记项目为失败状"
                logger.error("字幕文件不存在且Whisper生成失败，项目将标记为失败状")
        ...
        try:
            ...
            project.description = f"从B站下: {video_info.title}"
