...
from backend.utils.video_editor import VideoEditor

from backend.core.path_utils import get_data_directory, get_projects_directory
...
def get_video_editor() -> VideoEditor:
    ...
    from backend.core.shared_config import CLIPS_DIR, COLLECTIONS_DIR
...
async def get_clip_subtitles(
    ...
    try:
        ...
        if not clip:
            ...
            raise HTTPException(status_code=404, detail="片段不存")
        ...
        if not srt_file.exists():
            ...
            raise HTTPException(status_code=404, detail="字幕文件不存")
        ...
        # 如果start_time和end_time是整数（秒），直接使?
...
async def edit_clip_by_subtitles(
    ...
    try:
        ...
        if not video_files:
            ...
            raise HTTPException(status_code=404, detail="原始视频文件不存")
...
async def get_edited_video(
    ...
    try:
        ...
        if not edited_video_path.exists():
            ...
            raise HTTPException(status_code=404, detail="编辑后的视频文件不存")
...
async def get_preview_segment(
    ...
    try:
        ...
        if not preview_file.exists():
            ...
            raise HTTPException(status_code=404, detail="预览文件不存")
