...
# 导入依赖
from ..utils.video_processor import VideoProcessor, keyframe_aligner_available
...
class VideoGenerator:
    ...
    def generate_clips(self, clips_with_titles: List[Dict], input_video: Path, 
        ...
        # 批量生成切片（使用并行处理提升速度）
        # 注意：batch_extract_clips_parallel 会修改 clips_data 中的时间（关键帧对齐和静音处理在并行前完成）
        ...
        if keyframe_aligner_available and input_video.exists():
            try:
                ...
                aligner = KeyframeAligner(
                    ...
                    lazy_load=False  # 立即初始化以获取统计信息
                )
                
                # 生成对齐报告
                report_path = self.metadata_dir / "keyframe_alignment_report.json"
                ...
                logger.info(f"  - 视频时长: {report['video_duration']:.2f}s")
                logger.info(f"  - 关键帧数量: {report['keyframe_stats']['count']}")
                logger.info(f"  - 平均间隔: {report['keyframe_stats']['avg_interval']:.2f}s")
                
            except Exception as e:
                logger.warning(f"生成关键帧对齐报告失败: {e}")
    ...
    def _generate_smart_clips(self, clips_with_titles: List[Dict], input_video: Path, srt_path: Path) -> List[Path]:
        ...
        try:
            ...
            # 批量生成切片
            successful_clips, _ = self.video_processor.batch_extract_clips(input_video, clips_data)
