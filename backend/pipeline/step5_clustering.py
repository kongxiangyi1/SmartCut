...
class ClusteringEngine:
    ...
    def cluster_clips(self, clips_with_titles: List[Dict]) -> List[Dict]:
        ...
        try:
            # 调用大模型进行聚类
            response = self.llm_manager.call_with_retry(full_prompt, {'clips': clips_for_clustering})
            
            # 解析JSON响应
            collections_data = self.llm_manager.parse_json_response(response)

            if not collections_data:
                raise ValueError("LLM返回结果为空或解析失败")
